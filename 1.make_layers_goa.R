...
# software_types <- c("globe", "timezero", "opencpn") This hashed out because we only need Timezero software types for ADFG
...
for(ii in 1:length(software_types)) {
  ...
  trawlable_centroid <- sf::st_centroid(sf::st_make_valid(trawlable_grid))
  ...
  # allocation_df <- sf::st_read(here::here("assets", "data", "allocation", "goa_2025_station_allocation_450_aea.gpkg")) |>
  #   sf::st_drop_geometry()
  # 
